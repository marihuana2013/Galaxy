...
let item_descriptions = [
    'Тарілка з принтом Марса. Доступна в червоному та білому кольорах',
...
let merch_items = document.querySelectorAll('.merch-item');

for (let i = 0; i < merch_items.length; i++) {
    let descriptionElement = merch_items[i].querySelector('.merch-item-descr');
    let priceElement = merch_items[i].querySelector('.merch-item-price');
    descriptionElement.textContent = item_descriptions[i];
    priceElement.textContent = item_prices[i] 
}
...
function incrementCart() {
    let currentCount = parseInt(cartAmount.textContent);
